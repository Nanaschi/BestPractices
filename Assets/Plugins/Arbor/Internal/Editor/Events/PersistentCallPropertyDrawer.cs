				...
				if (_LayoutArea.IsDraw(methodNameRect))
				{
					...
					if (EditorGUI.EndChangeCheck())
					{
						if (newMemberInfo != memberInfo || isMissing)
						{
							...
							call.property.serializedObject.Update();
						}
					}
				}
					...
					case TargetMode.Slot:
						{
							...
							DataSlot slot = targetSlotProperty.slot;
							if (slot != null)
							{
								ClassConstraintInfo constraintInfo = slot.overrideConstraint;
								if (constraintInfo != null && constraintInfo.baseType != targetType)
								{
									constraintInfo.baseType = targetType;
								}
								else if (constraintInfo == null)
								{
									slot.overrideConstraint = new ClassConstraintInfo() { baseType = targetType };
								}
							}
