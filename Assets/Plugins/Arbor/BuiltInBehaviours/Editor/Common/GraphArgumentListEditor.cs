...
namespace ArborEditor
{
	...
	abstract class ExternalArgumentPopupBase : PopupWindowContent
	{
		...
		protected virtual void DoGUI()
		{
			...
			if (_LayoutArea.IsDraw(popupRect))
			{
				EditorGUI.BeginChangeCheck();
				Parameter.Type parameterType = ParameterTypeMenuItem.Popup(popupRect, EditorGUITools.GetTextContent("Parameter Type"), _ParameterType);
				...
				case Parameter.Type.ComponentList:
					{
						...
						if (_LayoutArea.IsDraw(lineRect))
						{
				...
				case Parameter.Type.VariableList:
					{
						...
						if (_LayoutArea.IsDraw(lineRect))
						{
							_ReferenceType = ArgumentGUITools.VariableListTypePopup(lineRect, _ReferenceType, EditorGUITools.GetTextContent("Reference Type"));
						}
					}
					break;
			}
		...
		void DoGUI(SerializedProperty elementProperty)
		{
			...
			if (isPublicSet)
			{
				switch (parameterType)
				{
					...
					case Parameter.Type.Component:
						{
							FlexibleComponentProperty flexibleComponentProperty = new FlexibleComponentProperty(parameterProperty);
							flexibleComponentProperty.overrideConstraintType = valueType;
						}
						break;
					case Parameter.Type.Enum:
					case Parameter.Type.AssetObject:
